/*
  VGA colour video generation - Multi-Bar Batch Swipe
  - Precise inter-phase pause between direction changes
...
int       hSpacingBytes   = 60;    // distance between vertical bars in bytes //60 means the first bar must completely leave before second bar appears
const int vThicknessRows  = 3;     // 3 rows = 3*16 = 48 px tall (rows)
int       vSpacingRows    = 33;    // distance between horizontal bars in rows

float barSpeed;
float setBarSpeed  = 0.15625f;   // CHANGE 1: was 0.15625f; x3 for same visual speed at 20Hz
...
bool colorFlicker = false; // toggles whether the checkerboard flickers between black and white or not
...
const int FLICKER_TOGGLE_FRAMES = 15;  // 60Hz / 15 = 4 toggles/sec = 6Hz full cycle
...
// CHANGE 2: divide by 3 because advanceFrame() now runs at 20Hz.
// iround(60*10/3) = 200 logic ticks. 200/20Hz = 10 seconds. Same wall time.
int  interPairPauseFrames     = iround(FPS * 30.0f / 3.0f);
...
int  interPhasePauseFrames     = iround(FPS * 20.0f / 3.0f);
...
void initBatch() {
  ...
  int head, spacing;
  if (isHorizontal()) {
    head    = (direction == LEFT_TO_RIGHT) ? -hThicknessBytes : (horizontalBytes  + hThicknessBytes);
    spacing = hSpacingBytes;
  } else {
    head    = (direction == TOP_TO_BOTTOM) ? -vThicknessRows : (verticalLines  + vThicknessRows);
...
inline int floorInt(float x) {return  (int)x - (x < 0 && x != (int)x ? 1 : 0); }
// ---------------- Draw bars into framebuffer ----------------
void drawBarsToBuffer() {
  clearFrameBuffer();

  for (int b = 0; b < barsPerBatch; b++) {
    int pos = floorInt(barPositions[b]);
      ...
      for (int t = 0; t < hThicknessBytes; t++) {
        ...
        if (x >= 0 && x < horizontalBytes) {
          for (int y = 0; y < verticalLines; y++) {
            if (((((y >> V_SHIFT) + (x >> H_SHIFT)) & 1) ^ checkerInvert) == 0) { //if (((((y >> V_SHIFT) + (x >> H_SHIFT)) & 1) ^ checkerInvert) == 0)
...
//      return true;
//    } else {
//      for (int i = 0; i < barsPerBatch; i++) if (barPositions[i] > -hThicknessBytes) return false;
//      return true;
//    }
//  } else {
...
//  }
  if (isHorizontal()) {
    if (direction == LEFT_TO_RIGHT) {
      for (int i = 0; i < barsPerBatch; i++)
        if (barPositions[i] < horizontalBytes) return false;
      return true;
    } else {
      for (int i = 0; i < barsPerBatch; i++)
        if (barPositions[i] > 0) return false;
      return true;
    }
  } else {
    if (direction == TOP_TO_BOTTOM) {
      for (int i = 0; i < barsPerBatch; i++)
        if (barPositions[i] < verticalLines) return false;
      return true;
    } else {
      for (int i = 0; i < barsPerBatch; i++)
        if (barPositions[i] > 0) return false;
...
// CHANGE 5: all "newFrame = false" lines removed. newFrame is now always
// cleared in loop() so it doesn't matter whether advanceFrame() runs or not.
void advanceFrame() {
  ...
  if (interPairHold) {
    if (interPairFramesRemaining > 0) {
      ...
      return;  // <-- removed "newFrame = false" here
    } else {
      interPairHold = false;
...
void loop() {
  ...
  while (true) {
    ...
    if (doOneScanLine()) {
      ...
      // CHANGE 4: newFrame cleared HERE, unconditionally, every VGA frame.
      // This is the critical fix. Previously it was only cleared inside
      // advanceFrame(), so on the 2 skipped frames it was never cleared,
      // causing doOneScanLine() to return true on every scanline and
      // updateCameraTriggerFrameTick() to fire hundreds of times per frame.
      newFrame = false;

      if (colorFlicker && running && !interPhaseHold && !interPairHold) {
        flickerFrameCount++;
